...
def circle_pathing(photo = None, imagePath="Stage1_circle_ref.png", visualize=False):
    ...
    circleBoundary = contours_sorted_by_area[1]
...
def square_pathing(photo = None, imagePath="Stage1_square_ref.png", visualize = False):
    ...
    redImg = (((R > 100) & (G < 70) & (B < 70)) * 255).astype(np.uint8)
    ...
    if len(contours_sorted_by_size) >= 2:
        squareBoundary = contours_sorted_by_size[1]
    else:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        circle_pathing(frame, visualize=True)
